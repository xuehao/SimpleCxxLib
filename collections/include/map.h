...
private:
    ...
    void mapAll(BSTNode* t, void (*fn)(const KeyType&, const ValueType&)) const {
        if (t != nullptr) {
            mapAll(t->left, fn);
            fn(t->key, t->value);
            mapAll(t->right, fn);
...
template <typename KeyType, typename ValueType>
void Map<KeyType, ValueType>::mapAll(void (*fn)(const KeyType&, const ValueType&)) const {
    mapAll(root, fn);
}
...
template <typename KeyType, typename ValueType>
std::ostream& operator<<(std::ostream& os, const Map<KeyType, ValueType>& map) {
    os << "{";
...
std::istream& operator>>(std::istream& is, Map<KeyType, ValueType>& map) {
    ...
    if (ch != '}') {
        ...
        while (true) {
            ...
            is >> ch;
            if (ch != ':')
                error("operator >>: Missing colon after key");
            ValueType value;
